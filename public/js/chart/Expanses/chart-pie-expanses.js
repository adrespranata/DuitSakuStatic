...
fetch('/getExpenseByCategory')
    ...
    .then(data => {
        ...
        var categoryPromises = data.map(item => {
            return getCategoryNameExpenses(item.category_id)
                .then(categoryName => {
                    return {
                        label: categoryName,
                        value: item.total
    ...
    .then(categoryData => {
        ...
        var ctx = document.getElementById("expansesPieCharts");
        var expansesPieCharts = new Chart(ctx, {
            type: 'doughnut',
            data: {
        ...
        var additionalLabelsDiv = document.getElementById('additionalLabelsExpanses');
...
function getCategoryNameExpenses(categoryId) {
    return fetch(`/getCategoryNameExpenses/${categoryId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
